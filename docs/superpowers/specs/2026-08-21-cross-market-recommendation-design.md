...
| 3 | 数据刷新/评估 | 程序每天刷新数据、惰性天级缓存（按自然日缓存），不引入调度器；用户操作频率为周级（仅影响查看习惯，不影响程序） |
...
| 8 | 投资周期 | 数据日频刷新；基金 1-2 年、股票/外汇/加密按默认评价窗口（可配置） |
...
class AnalysisContext:
    ...
    horizon: str                # 评价窗口档位：short / medium / long（数据本身日频刷新）
```
...
数据本身**日频刷新**（每天更新），评价窗口指「用多长历史来算收益/风险」：

| 资产类别 | 评价窗口（默认） | 档位 |
|---|---|---|
| 股票（A股/美股/韩股/港股） | 日频，20-60 交易日 | short |
| 基金 | 1-2 年 | medium/long |
| 外汇 | 日频，60-120 交易日 | short |
| 数字货币 | 日频，30-90 交易日 | short |

评价窗口影响两点：① 取多长历史算收益/风险；② 持有期约束（短期资金不推需长持的基金）。
