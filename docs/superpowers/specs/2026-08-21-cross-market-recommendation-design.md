...
| 3 | 周级评估 | 惰性周级缓存（按 ISO 周缓存），不引入调度器 |
...
| 8 | 投资周期 | 股票周级、基金 1-2 年、外汇/加密按默认表（可配置） |
...
class AnalysisContext:
    ...
    horizon: str                # 投资周期档位：weekly / medium / long
```
...
| 资产类别 | 评价窗口（默认） | 周期档位 |
|---|---|---|
| 股票（A股/美股/韩股/港股） | 周频，26-52 周 | weekly |
| 基金 | 1-2 年 | medium/long |
| 外汇 | 周频，26-52 周 | weekly |
| 数字货币 | 周频，12-26 周 | weekly |
...
- 不引入调度器。
- 每个标的按「ISO 周」缓存评估结果：同一周内重复查询复用，跨周自动重算。
- 实现：`search.py` 已含 `daily_cache_get/set`，改为按 `ISO 年-周`（如 `2026-W34`）判定。
